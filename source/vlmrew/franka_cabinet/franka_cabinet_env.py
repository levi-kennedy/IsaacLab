...
class FrankaCabinetEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: FrankaCabinetEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.vlm_image_frames_array = []
        self.vlm_reward_buf = torch.zeros(self.num_envs, device=self.device)
        self.chkpnt_step_cnt = kwargs.get("chkpnt_step_cnt", 0)
    ...
    def _setup_scene(self):
        ...
        self._table_camera = Camera(self.cfg.table_camera)
    ...
    def _get_observations(self) -> dict:
        ...
        if self.num_envs == 1:
            ...
            # convert the table camera image for each robot environment to b64 and save in a list save frame
    ...
    def _compute_vlm_rewards(self):
        ...
        if self.common_step_counter - 1 in save_frame_idx and len(self.vlm_image_frames_array) > 1:
            ...
            # we are stacking arrays of images for each environment and if we have more than one then grab the first one
            ...
            try:
                ...
                # move to the gpu
                self.vlm_reward_buf = self.vlm_reward_buf.to(self.device)
                # make sure the buffer is the correct size
                if self.vlm_reward_buf.shape[0] != self.num_envs:
                    self.vlm_reward_buf = torch.zeros(self.num_envs, device=self.device)
            except:
                self.vlm_reward_buf = torch.zeros(self.num_envs, device=self.device)
